    ...
    for pass_iteration in range(numpasses):
        ...
        for bow, goldlabel in examples:
            ...
            if predlabel != goldlabel:
                ...
                f1=features_for_label(bow,predlabel)
                f2=features_for_label(bow,goldlabel)
                diff=dict_subtract(f2,f1)
                for label,values in diff.iteritems():
                    weights[label]+=diff[label]
                    S[label]+=(t-1)*diff[label]
        t+=1
                #pass
        print "TR RAW EVAL:",
...
def plot_accuracy_vs_iteration(train_acc, test_acc, avg_test_acc, naive_bayes_acc):
    ...
    NB=np.ones(10)*naive_bayes_acc
    plt.plot(range(10),train_acc,'r',label='Training')
    plt.plot(range(10),test_acc,'b',label='Test')
    plt.plot(range(10),avg_test_acc,'g',label='Avg Test')
    plt.plot(range(10),NB,'y',label='Naive Bayes')
    plt.ylabel('Accuracy')
    plt.xlabel('Iterations')
    plt.title('Accuracy Vs Iterations graph for models')
    plt.legend()
    plt.show()
...
if __name__=='__main__':
    ...
    plot_accuracy_vs_iteration(sol_dict['train_acc'], sol_dict['test_acc'], sol_dict['avg_test_acc'],.8294)
