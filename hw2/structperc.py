...
import vit_starter as vit  # your vit.py from part 1
...
def train(examples, stepsize=1, numpasses=10, do_averaging=False, devdata=None):
    ...
    def get_averaged_weights():
        ...
        for label,values in S.iteritems():
            weight[label] =weights[label]-S[label]*1.0/t
        return weight
...
#        random.shuffle(examples)
        for tokens,goldlabels in examples:
            pred=predict_seq(tokens,weights)
            
            if t==numpasses-1 and c<2:
                show_predictions(tokens,goldlabels,pred)
                c+=1
            if pred!=goldlabels:
                f1=features_for_seq(tokens,pred)
                f2=features_for_seq(tokens,goldlabels)
                diff=dict_subtract(f2,f1)
                for key,value in diff.iteritems():
                    weights[key]+=diff[key]
                    S[key]+=(t-1)*diff[key]
        t+=1
#        print weights
...
def features_for_seq(tokens, labelseq):
    ...
    for i in range(len(labelseq)):
        for j in OUTPUT_VOCAB:
            if labelseq[i]==j:
                temp1=local_emission_features(i,labelseq[i],tokens)
                feat.update(temp1)
        if c!=0:
            feat['trans_%s_%s'%(labelseq[i-1],labelseq[i])]=1
        c+=1
    return feat
...
def calc_factor_scores(tokens, weights):
    ...
    Ascores = { (tag1,tag2): weights.get(('trans_%s_%s')%(tag1,tag2),0) for tag1 in OUTPUT_VOCAB for tag2 in OUTPUT_VOCAB }
    Bscores = []
    for t in range(N):
        # IMPLEMENT THE INNER LOOP
...
if __name__ == '__main__':
    ...
    traind=read_tagging_file('oct27.train')
    devd=read_tagging_file('oct27.dev')
    result=train(traind,do_averaging=True,devdata=devd)
    do_evaluation(devd,result)
    fancy_eval(devd,result)
